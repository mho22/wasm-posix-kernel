/**
 * Erlang/OTP runtime filesystem bundle loader.
 *
 * Loads a pre-built OTP bundle (JSON manifest + file data) into
...
/**
 * Load an Erlang/OTP runtime bundle into the given MemoryFileSystem.
 */
export async function loadErlangBundle(
  fs: MemoryFileSystem,
  bundleUrl: string,
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to load Erlang bundle from ${bundleUrl} (${response.status}). ` +
      `Run: bash examples/browser/scripts/build-erlang-bundle.sh`
    );
  }
