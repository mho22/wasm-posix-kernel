/**
 * Perl stdlib filesystem bundle loader.
 *
 * Loads a pre-built Perl stdlib bundle (JSON manifest + file data) into
 * the kernel's MemoryFileSystem. Same format as python-bundle.
 *
...
/**
 * Load a Perl stdlib bundle into the given MemoryFileSystem.
 */
export async function loadPerlBundle(
  fs: MemoryFileSystem,
  bundleUrl: string,
  ...
  if (!response.ok) {
    throw new Error(
      `Failed to load Perl stdlib bundle from ${bundleUrl} (${response.status}). ` +
      `Run: bash examples/browser/scripts/build-perl-bundle.sh`
    );
  }
