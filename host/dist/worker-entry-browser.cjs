...
sw.onmessage = (e) => {
  const data = e.data;
  const port = {
    postMessage: (msg, transfer) => sw.postMessage(msg, transfer),
    on: (event, handler) => {
      if (event === "message") {
        sw.onmessage = (ev) => handler(ev.data);
      }
    }
  };
  if (data.type === "centralized_init") {
    centralizedWorkerMain(port, e.data).catch((err) => {
      console.error(`[worker-entry-browser] centralizedWorkerMain error pid=${data.pid}:`, err);
    });
  } else if (data.type === "centralized_thread_init") {
    centralizedThreadWorkerMain(port, e.data).catch((err) => {
...
//# sourceMappingURL=worker-entry-browser.cjs.map