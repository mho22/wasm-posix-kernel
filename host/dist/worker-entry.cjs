...
if (import_node_worker_threads.parentPort) {
  import_node_worker_threads.parentPort.on("message", () => {
  });
  const data = import_node_worker_threads.workerData;
  if (data.type === "centralized_init") {
    centralizedWorkerMain(import_node_worker_threads.parentPort, import_node_worker_threads.workerData).catch((e) => {
      console.error(`[worker-entry] centralizedWorkerMain error: ${e}`);
    });
  } else if (data.type === "centralized_thread_init") {
    try {
      centralizedThreadWorkerMain(import_node_worker_threads.parentPort, import_node_worker_threads.workerData).catch((e) => {
