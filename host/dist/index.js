...
var NodeWorkerAdapter = class {
  ...
  constructor(entryUrl) {
    this.entryUrl = entryUrl ?? new URL("./worker-entry.ts", import.meta.url);
  }
  /**
  ...
  createWorker(workerData) {
    ...
    if (compiledEntry) {
      const worker2 = new Worker(compiledEntry, { workerData });
...
function findRepoRoot(startFrom) {
  ...
  const here = startFrom ?? (import.meta.url ? dirname(fileURLToPath(import.meta.url)) : process.cwd());
  let dir = resolve2(here);
  for (let i = 0; i < 20; i++) {
    if (existsSync2(join2(dir, "binaries.lock")) && existsSync2(join2(dir, "abi/manifest.schema.json"))) {
...
function resolveBinary(relPath) {
  ...
  const local = join2(repo, "local-binaries", adjusted);
  if (existsSync2(local)) return local;
  const fetched = join2(repo, "binaries", adjusted);
  if (existsSync2(fetched)) return fetched;
  throw new Error(
...
function binariesDir() {
  return join2(findRepoRoot(), "binaries");
}
function localBinariesDir() {
  return join2(findRepoRoot(), "local-binaries");
}
...
import { Worker as NodeThreadWorker } from "worker_threads";
...
function spawnKernelWorkerThread() {
  const entryTs = join3(__dirname, "node-kernel-worker-entry.ts");
  const entryJs = join3(__dirname, "node-kernel-worker-entry.js");
  const distJs = entryTs.replace(/\/src\/([^/]+)\.ts$/, "/dist/$1.js");
  if (existsSync3(distJs)) {
    return new NodeThreadWorker(distJs);
  }
  if (existsSync3(entryJs)) {
    return new NodeThreadWorker(entryJs);
  }
  const require2 = createRequire2(import.meta.url);
  const tsxApiPath = require2.resolve("tsx/esm/api");
  const tsxApiUrl = pathToFileURL2(tsxApiPath).href;
  const entryUrl = pathToFileURL2(entryTs).href;
  const bootstrap = [
    `import { register } from '${tsxApiUrl}';`,
    `register();`,
    `await import('${entryUrl}');`
...
export {
  BrowserTimeProvider,
  CH_TOTAL_SIZE2 as CH_TOTAL_SIZE,
